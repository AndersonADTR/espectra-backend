...
class RedisService {
    ...
    constructor() {
        this.client = new ioredis_1.default({
            host: config_service_1.config.getRequired('REDIS_HOST'),
            port: config_service_1.config.get('REDIS_PORT', 6379),
            connectTimeout: 5000,
            commandTimeout: 3000,
    ...
    async set(key, value, ...args) {
        ...
        try {
            return await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error('Redis set operation timed out')), 5000))
            ]);
        }
        catch (error) {
    ...
    async get(key) {
        ...
        try {
            return await Promise.race([
                this.client.get(key),
                new Promise((_, reject) => setTimeout(() => reject(new Error('Redis get operation timed out')), 5000))
            ]);
        }
        catch (error) {
    ...
    async del(...keys) {
        ...
        try {
            return await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error('Redis del operation timed out')), 5000))
            ]);
        }
        catch (error) {
    ...
    async sadd(key, ...members) {
        ...
        try {
            return await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error('Redis sadd operation timed out')), 5000))
            ]);
        }
        catch (error) {
    ...
    async exists(key) {
        ...
        try {
            return await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error('Redis exists operation timed out')), 5000))
            ]);
        }
        catch (error) {
