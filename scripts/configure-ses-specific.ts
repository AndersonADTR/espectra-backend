// scripts/configure-ses-specific.ts
...
// Verificar ambos correos
console.log(`Configurando SES para verificar los correos: ${senderEmail} y ${recipientEmail}`);

// Ejecutar configuración para el correo de origen
console.log('\n--- Configurando correo de origen ---');
configureSES(senderEmail, userPoolId, region)
  .then(() => {
    // Ejecutar configuración para el correo de destino
    console.log('\n--- Configurando correo de destino ---');
    return configureSES(recipientEmail, userPoolId, region);
  })
  .catch(console.error);
